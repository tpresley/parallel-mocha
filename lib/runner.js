...
Runner.prototype.run = function(callback) {
  ...
  pool.on('ready', function(file, callback) {
    ...
    self.spawn(file, self.index, row, callback);
    self.index++;
  });
  pool.start();
};

Runner.prototype.spawn = function(file, testNumber, row, callback) {
  var self = this
  ...
  setTimeout(function(row){ return function(){
    ...
    child.on('exit', function(code) {
      ...
      process.stderr.write('[' + self.config.index + ':' + testNumber + ':DONE]\n')
      callback(code);
    });
